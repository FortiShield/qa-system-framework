"""
Module to build the Fortishield socket paths.

This modules contains the following:

- SocketPath(FortishieldPath:
...
class SocketPath(FortishieldPath):
    """Class to build the fortishield socket paths.
    ...
    Attributes:
        os_system (str): Operating system.
        sockets_path (str): Fortishield logs path.
    """
    def __init__(self, os_system=sys.platform):
        super().__init__(os_system)
        self.sockets_path = os.path.join(self.get_fortishield_path(), 'queue', 'sockets')
